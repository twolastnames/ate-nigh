...
if (process.argv.length < 3) {
  console.error(
    "Error: please include a non-existant file/directory name on the command line",
  );
}
...
const outputFile = (output: OutputType, context: ContextType) => {
  ...
  const template = readFileSync(
    output.template.replaceAll("/", path.sep),
  ).toString();
...
const dumpContext = (context: ContextType) =>
  Object.entries(context)
    .map(([key, value]) => `${key}: ${value}`)
    .join("\n");

process.argv.forEach(function (input, index) {
  if (index < 2) {
    return;
  }
