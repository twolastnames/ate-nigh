...
const types = Object.keys(schemas.types).concat(
  Object.keys(schemas.collections),
);
...
for (const path of paths) {
  ...
  const aggregation = schemas.aggregations.find(
    ({ name }: { name: string }) => name === path,
  );
  const backPath = path
    .split("/")
    .map(() => "../")
  ...
  writeFileSync(
    ...
    ejs.render(readFileSync("./templates/route.ts.ejs").toString(), {
      path,
      backPath,
      types,
      schemas,
...
writeFileSync(
  `./hooks/api.ts`,
  ejs.render(readFileSync("./templates/apiHooks.ts.ejs").toString(), {
    schemas,
    types,
    dumpParametersTypescript,
    dumpTypescriptSchema,
